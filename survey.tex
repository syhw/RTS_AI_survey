...
Note that all qualified bots play Protoss.
...
There are a set of problems that are either solved in RTS games (path-finding), or for which we have very good solutions (micro management). But there are features that are not exhibited by any of the current Starcraft playing bots. For example, current bots are incapable of coming up on their own with tactical moves such as unit drops. Some bots do drops, but only if this is hard-coded; no bot has the capability of reasoning about all the possible moves available, and discovering that doing a drop is the best thing to do right now. This is related to the lack of real-time adversarial planning techniques that scale up to RTS games. Similarly, current bots do not exhibit adaptation to opponent strategies. Some switch between different build-orders, but do not fully adapt their strategy. No bot is capable of observing the opponent and autonomously come up with a good plan (from scratch) to counter the opponent strategy.

In general, this is a list of open problems:

\begin{itemize}
\item Adaptation to opponent
\item Learning (from opponent, from traces or from mistakes)
\item Scalability: this includes:
\begin{itemize}
\item a) how to scale up existing algorithms for adversarial domains like game tree search or others, and 
\item b) multi-scale planning/reasoning, how to integrate control, tactics and strategy together (e.g. how to interface them).
\end{itemize}
\item Adversarial real-time planning and scheduling under uncertainty with resources: this basically includes three general unsolved AI problems:
\begin{itemize}
