...
{\color{red} A typical Starcraft map is defined as a rectangular grid,
...
exploitation.  This  involves  buildings  placing  where  one  has  to
...
configurations  where large  units could  be stuck.  Base expansion
also belongs  to spatial reasoning,  in the sense  that the AI  has to
...
closely related to  tactics with questions like ``Where  should I place
my  units for battle?'',  favoring for  instance engagements  when the
opponent's units are into a  bottleneck. In Starcraft, it is always an
advantage to  have its units  on high ground  whereas enemy is  on low
ground, since units  on low ground have no vision  on high ground, and
firing from low  ground have only 70\% of chance to  hit units on high
ground.

As well as spatial reasoning, temporal reasoning is related to tactics
since it implies  to decide about army movements,  like when attacking
or  withdrawing  units from  battle.   A  part  of temporal  reasoning
consists  of  threat predictions,  where  one  has  to plan  when  the
opponent is  likely running an  attack.  Strategy copes  with temporal
reasoning  as well,  mainly with  planning problems:  when researching
upgrades, when constructing  wished buildings, when switching strategy
if needed, etc. 
...
Playing a  RTS game like Starcraft  can be decomposed  into three main
tasks: 
\begin{itemize}
\item Strategy:  corresponds to the  decision making process.  This is
  ...
  set of  units and buildings  one owns. If  we may allow  ourselves a
  military analogy, strategy is orders given by generals at the HQ. 
\item  Tactics: are  the implementation  of the  current  strategy. It
  implies  army and  building positioning,  movements, timing,  and so
  on.  Tactics concerns a  group of  units. They  are orders  given by
  officers on the battlefield.
\item  Reactive  control:  is  the  implementation  of  tactics.  This
  consists in   moving,    targeting,   firing,    fleeing   during
  battle. Reactive control focuses on  a specific unit. It is soldiers
  applying officers' orders.
\end{itemize}
...
\begin{itemize}
\item Terrain analysis: consists  in the analysis of regions composing
  the  map,  chokes,  minerals  and  gas emplacements,  low  and  high
  walkable grounds, islands, etc.
\item Intelligence gathering:  corresponds to information collected on
  our  opponent. Because of  the fog  of war,  a non-cheating  AI must
  regulary send scouts to localize and spy enemy bases, and must see or
  infer what buildings and technology the enemy has developed in order
  to adapt its strategy.
\end{itemize}

\noindent
The reader can find a  good presentation of task decomposition for AIs
playing RTS in~\cite{weber2011acs}.  }
...
while all of them are  reactive at the lower level (reactive control),
most  if  not all  of  them,  are scripted  at  the  highest level  of
abstraction. BTHAI reads build  and squad formations from a predefined
script,  Nova's {\em  Strategy Manager}  is a  predefined finite-state
machine,  BBQ's construction  manager  reads the  build  order from  a
predefined script, and Skynet's  {\em BuildOrder Manager} is basically
a predefined script. Such scripts  describe the strategy that the bots
will use, however,  such strategy is always fixed.  One could see this
pre-scripting  as  if  each  bot defined  a  ``high-level  programming
language'' to  describe Starcraft strategies, and  the bots themselves
are just interpreters of such strategy. Compared to current approaches
for  Chess or Go,  this scripting  seems a  rigid and  inflexible, but
responds  to the  much higher  complexity  of the  Starcraft game.  An
interesting  exception to  that is  UAlbertaBot, which  uses  a search
algorithm in  the {\em Production Manager} to  find near-optimal build
orders.  Another interesting  case  is  AIUR, that  uses  a {\em  Mood
  Manager}  to   randomly  pick  a  mood  among   six  (cheese,  rush,
aggressive, defensive,  macro, fast expand), which  will influence the
build order,  strategy and tactics. All  of them scripted  so far, but
build  orders  can been  modified  but  the  Spend Manager,  following
available resources.  This mood can change during  the game, regarding
the opponent behavior.
